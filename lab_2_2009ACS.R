...
options(scipen=999)
options(tigris_class = "sf") # tigris_class tells tidycensus to download Census geometries in the sf or Simple Feature format.
...
B <- 
  ggplot() +
  geom_sf(data = totalPop09, aes(fill = q5(estimate))) + # use quintile break
  theme(plot.title = element_text(size=22))
